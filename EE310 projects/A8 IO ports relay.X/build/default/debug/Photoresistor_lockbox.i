...
void Emergency_Initialize(void);
...
_Bool PR1_IsActive(void);
_Bool PR2_IsActive(void);


void Reset_InputData(void);
void Reset_To_Start(void);


void Update_SensorsAndCounts(void);
...
void __attribute__((picinterrupt(("irq(7), base(8)")))) ISR_IOC(void)
{
    ...
    if (PIR0bits.IOCIF && IOCBFbits.IOCBF0)
    {
        ...
        Check_Emergency_SW = 1;
        SystemState = Emergency_Pressed;
...
void Update_SensorsAndCounts(void)
{
    _Bool pr1_active = PR1_IsActive();
    _Bool pr2_active = PR2_IsActive();
    ...
    switch (SystemState)
    {
        case waitFor_PR1:
        {
            if (pr1_active && !PR1Prev && (PR1_Debounce == 0))
            {
                if (PR1_Count < 4U)
                {
                    PR1_Count++;
                    Seg7_Display(PR1_Count);
                }

                PR1_DONE = 0;
                PR1_Debounce = (60U / 10U);
            }
        ...
        case STATE_WAIT_PR2:
        {
            if (pr2_active && !PR2Prev && (PR2_Debounce == 0))
            {
                if (PR2_Count < 4U)
                {
                    PR2_Count++;
                    Seg7_Display(PR2_Count);
                }

                PR2_DONE = 0;
                PR2_Debounce = (60U / 10U);
            }
...
void Process_System(void)
{
    switch (SystemState)
    {
        case waitFor_PR1:
        {
            ...
            if ((PR1_Count > 0U) && (PR1_DONE >= (1500U / 10U)))
            {
        ...
        case STATE_WAIT_PR2:
        {
            if ((PR2_Count > 0U) && (PR2_DONE >= (1500U / 10U)))
            {
        ...
        {
            if ((PR1_Count == 3U) && (PR2_Count == 2U))
            {
                SystemState = Correct_Secret_Code;
            }
            else
            {
                SystemState = Wrong_Secret_Code;
            }
            break;
        }
