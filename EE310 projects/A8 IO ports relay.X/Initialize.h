 ...
 */
#define SYS_LED_LAT     LATBbits.LATB3      // writes a 1 or 0 to the LED output pin
#define SYS_LED_TRIS    TRISBbits.TRISB3    // decides whether that LED pin behaves as input or output

/* Photoresistor input pins:
 * these two pins receive the sensor voltages from the two LDR divider circuits.
 */
#define PR1_PORT        PORTBbits.RB1       // reads the actual logic level seen on PR1
#define PR1_TRIS        TRISBbits.TRISB1    // configures PR1 pin direction

#define PR2_PORT        PORTBbits.RB2       // reads the actual logic level seen on PR2
#define PR2_TRIS        TRISBbits.TRISB2    // configures PR2 pin direction

/* Emergency switch pin:
 * this push button is used as the interrupt source for the emergency condition.
 */
#define E_SW_PORT     PORTBbits.RB0       // Emergency switch port
#define E_SW_TRIS     TRISBbits.TRISB0    // set E_SW port as input

/* Relay control pin:
 * this pin drives the input side of the relay module.
 */
#define RELAY_LAT       LATBbits.LATB5     // writes the control signal that turns the relay module ON or OFF
#define RELAY_TRIS      TRISBbits.TRISB5    // configures the relay control pin as output

#define BUZZER_LAT       LATBbits.LATB7     // writes the control signal that turns the relay module ON or OFF
#define BUZZER_TRIS      TRISBbits.TRISB7    // configures the relay control pin as output
